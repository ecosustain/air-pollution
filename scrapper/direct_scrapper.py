...
def get_dates_to_update(df):
    ...
    count_max_date = (df['datetime'] == max_datetime).sum()
    if count_max_date > 1:
        start_date = (max_datetime + pd.Timedelta(days=1)).strftime("%d/%m/%Y")
    else:
        start_date = max_datetime.strftime("%d/%m/%Y")
...
def update_data(session_id):
    ...
    data_directory = "./data/scraped_data"
    for file in os.listdir(data_directory):
        ...
        for indicator in indicators:
            ...
            if ("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">" in response.text):
                continue
            update_database(response.text, station, indicator)
            print(f"Succesfully updated: {station} - {indicator} - up to {end_date}")
...
def update_database(data, station, indicator):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(file_path, "w") as file:
            file.write(data)
            update_measure_indicator_table(file_path, station, indicator, db_connection)
...
def update_measure_indicator_table(file_path, station, indicator, db_connection):
    ...
    df = adjust_columns_and_data(df, station, indicator)
